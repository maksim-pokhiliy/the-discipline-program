...
### §4.12 Step 8.3.7 — Schema dual-scope (blockId / parentSchemaId, order) positional uniqueness constraint

Added 2026-05-21 per Step 8.3.7 (mirrors §4.7 Block / §4.11 SchemaRow, across Schema's two positioning scopes). Schema has a dual scope — a top-level schema (`parentSchemaId IS NULL`) is positioned within its Block, a sub-schema (`parentSchemaId IS NOT NULL`) within its parent Schema — so the positional-uniqueness invariant needs two constraints with two enforcement mechanisms. Sub-schemas: Prisma `@@unique([parentSchemaId, order])` on the Schema model — the generated unique index spans every row, but Postgres treats every `NULL` as distinct, so two top-level rows (`parentSchemaId = NULL`) never collide under it whatever their `order`; the constraint therefore effectively binds only sub-schemas. Top-level schemas: a partial unique index `schemas_block_top_order` on `training_schemas ("blockId", "order") WHERE "parentSchemaId" IS NULL`, appended to `prisma/sql/lms-checks.sql` and applied by `apply-sql-checks.ts` after `prisma db push`. Prisma `@@unique` cannot express a `WHERE` predicate, and a flat `@@unique([blockId, order])` would be wrong — `blockId` is non-nullable on sub-schemas too, and two sub-schemas of different parents in one block legitimately share an `order`. As in §4.11, no api change is needed: `lmsSchemaApi.create` (`_max(order) + 10` per scope inside a `Serializable` transaction wrapped in `retryOnP2034`) and `lmsSchemaApi.reorder` (shipped Step 8.1a, commit `0d7c6943`, already in the canonical two-pass shift-to-negative form, dual-scope aware) are both compatible with zero changes — the intra-transaction trace confirms every intermediate UPDATE holds pairwise-distinct tuples under both constraints and both scopes. `schema/admin.ts` is byte-identical after this step. Future enhancement (deferred carry-forward QA-001c, codebase-wide): widen `retryOnP2034` to also retry P2002 on the `_max+N` insert pattern. Out of Step 8.3.7 scope.
