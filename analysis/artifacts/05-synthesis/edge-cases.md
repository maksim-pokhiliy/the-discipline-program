...
| Archetype                        | Block     | Decision Phase 5                                                                                                                                                             |
| -------------------------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| alternating-sets                 | block-009 | **first-class archetype** с `paired_with_schema_ref` archetype_param. Phase 6 — bidirectional FK persistence.                                                                |
...
| Q20 (Phase 7)               | super-set vs SchemaPairing reuse                         | archetype params (не SchemaPairing entity); SchemaPairing остаётся для alternating-sets                                                    | **CLOSED Phase 7** — archetype `super-set` + `ArchetypeSuperSetParams`                                        |
...
### 6.5 MEDIUM — paired_with_schema_ref persistence

**Q (Phase 5)**: alternating-sets archetype requires bidirectional FK между paired schemas (block-009).

**Action**: Phase 6 decides — direct FK on Schema (cyclic concerning DB constraints) vs separate `SCHEMA_PAIRING` join table.

**Recommended**: separate join table.
...
| Q20 | super-set vs SchemaPairing reuse      | **Archetype params** (NOT SchemaPairing entity). Archetype `super-set` (kind=ATOMIC, family=ROUNDS_SETS) с `ArchetypeSuperSetParams { pairs: SuperSetPair[], restBetweenPairs?: RestSpec, rounds: number }`. `SuperSetPair = { label, schemaRows: SchemaRowRef[] }`. SchemaPairing остаётся для alternating-sets (bidirectional schema relation). Super-set — ordered exercise sequence внутри одной schema. |
