...
| `MOBILE_PUBLISH_ENCRYPTION_KEY` | platform                              | AES-256-GCM key for the legacy mobile connector token-at-rest. Retiring at P4.1 (ADR-0043) — the platform itself serves the iOS app since the 2026-09-17 apex cutover; stays required until the connector edge is removed. |
| `LEGACY_MOBILE_API_BASE_URL`    | platform                              | Base URL of the retired legacy Spring backend the connector still dual-writes plans into (nobody reads it since the 2026-09-17 cutover; retiring at P4.1, required until then).                                            |
