...
1. **Carry `implementation/step-06.1/prompt.md` to an executor session.** Thesis ratified by user; prompt drafted with verbatim reads of Step 5 patterns (`week/admin.test.ts` TZ assertion shape, `plan-enrollment/admin.ts` CRUD template, `mapToWeek` mapper template, `verifyPlanOwnership` guard pattern, registration files current state). Six binding decisions (D-6.1-1..6) encoded as prompt guardrails; `freezeLoadsAtCreation` and `Session.name` regression checks specified at the grep level. Three planned commits (`refactor → feat → test`). Expected output: executor writes `implementation/step-06.1/output.md`; planner spot-checks; close-out IMPLEMENTATION_LOG entry; advance Step 6.2 thesis.
2. **After Step 6.1 closes — Step 6.2 thesis.** `lmsDayMetadataApi` (label + notes upsert side-channel; reuses `_shared/date.ts` extracted in 6.1) + `getWeekResponseSchema` extension to `{ week, days[7] }`. Server always returns 7 weekdays (empty slots = `{ ..., label: null, notes: null, sessions: [] }`). Consumes `dayOfWeekSchema` from `contracts/lms/_shared`. Verbatim read of Step 6.1 `lmsSessionApi` to understand the day-upsert pattern before specing `lmsDayMetadataApi`.
