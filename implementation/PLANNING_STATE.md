...
> Last updated: 2026-05-15 (Step 6.0 closed — Session contract slice + `lms/_shared` namespace landed in 4 commits + 1 docs commit on `feat/training-domain`; Step 6.1 thesis is the next planner action)
...
**Step 6.0 closed** (2026-05-15). HEAD on `feat/training-domain` past `71881b75 docs(step-06.0): close out session contract slice and _shared namespace`. 11 new files (`_shared/day-of-week.ts` + `_shared/day-of-week.test.ts` + 9 session-slice files including 3 test files) + 3 additive edits (`lms/index.ts`, `package.json` exports, `contracts/README.md`); +522/−1. 160 tests across 11 files (baseline 115/8). All 9 gates green (contracts type-check/lint/test, root type-check 16/16, root lint 16/16, dep:check 0 violations on 1105 modules, `contracts-no-prisma` holds). Two regression-guard tests added defensively beyond the spec — `shape.name === undefined` and `shape.freezeLoadsAtCreation === undefined` in both entity and write schemas — anti-drift guards for the indefinite deferred sub-decisions.
...
1. **Step 6.1 thesis** — `lmsSessionApi` (CRUD + reorder + sparse-order recompute) + extract `resolveWeekStartDate` from `endpoints/lms/week/admin.ts` into `endpoints/lms/_shared/date.ts` (now mandated by **two** callsites: Week notes-upsert and Session create's transitive Week materialization). Per read-then-spec the planner reads verbatim before drafting: `lmsWeekApi` (Step 5) as nested-resource handler template, `mapToWeek` as no-enum mapper template (Session has nullable FK to Label, otherwise plain copy), `Day` Prisma model + `(weekId, dayOfWeek)` unique constraint, the new `lms/session` contract slice from Step 6.0 (param schemas + write shapes). **Hard carry-forward: QA-001** — `lmsSessionApi.create` issues `prisma.session.create` with `day: { connectOrCreate: { …, week: { connectOrCreate: { … startDate UTC-midnight … } } } }`; both Week.startDate writes must use the extracted helper. Drafting: тезисы → user approval → `implementation/step-06.1/prompt.md`.
2. **After Step 6.1 closes — Step 6.2 thesis.** `lmsDayMetadataApi` (label + notes upsert side-channel) + `getWeekResponseSchema` extension to `{ week, days[7] }`. Server always returns 7 weekdays (empty slots = `{ ..., label: null, notes: null, sessions: [] }`). Consumes `dayOfWeekSchema` from `lms/_shared`.
