...
- **QA-907 — `z.string().url()` accepts `javascript:` / `data:` / `file:` schemes (HARD forward-risk for any clickable-URL step)** (INFO today; Step 9.2 QA, planner-traced 2026-05-22). `standaloneUrlRowFormSchema.url` is `z.string().url()` — a format check, not a safe-scheme allowlist; QA verified (against the project's real zod) it accepts `javascript:alert(1)`, `data:text/html,…`, `vbscript:`, `file:///…`. **In 9.2 this is harmless** — the `STANDALONE_URL` summary renders the url as a plain `Chip` label (React-escaped text, no `href`); QA traced the render path and confirmed XSS-safe. The risk is forward-looking. **Action — any future step that renders a `STANDALONE_URL` (or a row-level media URL) as a live `<Link href>` / `<a>` MUST gate the url through a `safeHttpUrl` helper** (allowlist `http:`/`https:` only, `rel="noopener noreferrer"`). The future clickable-URL step's thesis MUST carry this; do not ship a clickable url without the allowlist.

- **QA-904 — `formatRestRaw` appends `setIndex` unconditionally** (INFO; Step 9.2 QA). A `{ scope: "between_sets", setIndex: 2 }` rest (contract-valid — `setIndex` is an independent `.optional()` on `restSpecSchema`) yields the mildly nonsensical `"between sets 2"`; only `after_specific_set` + `setIndex` reads cleanly (`"after set 3"`). `raw` is non-empty and contract-valid — not a defect. **Action when triggered**: optional polish — append `setIndex` only when `scope === "after_specific_set"` (a `formatRestRaw` 1-line condition + a `format-rest-raw.test.ts` update — a behaviour + test change, not an inline fix). Effort XS.

- **QA-905 — `steps` / chip values above `2^53` pass the form schema + the contract with silent precision loss** (INFO; Step 9.2 QA — theoretical, QA-201-adjacent). `z.number().int().positive()` is `true` for all JS floats ≥ `2^53`; a 20-digit draft passes `parseStepDraft`'s `/^[1-9]\d*$/`, the form schema, and the contract `INNER_LADDER_MARKER.steps`, persisting a precision-lost number. No realistic path (rep counts are ≤ 3 digits). **Action when triggered**: a domain cap (`MAX_REASONABLE_STEP`) folded into the QA-201 param-bounds domain sub-step.

- **QA-906 — a `REST_SLOT` row's Edit menu item opens nothing** (INFO; Step 9.2 QA — pre-existing since 9.1). `SchemaRowCard` renders an Edit `MenuItem` for every row; for a `REST_SLOT` row (a valid `ROW_KIND`, not coach-creatable) the registry miss → `RowEditorModal` renders `null` → the click is a visible no-op. Pre-existing (the Edit menu has been unconditional since 9.1) — not a 9.2 regression. **Action when triggered**: hide/disable the Edit `MenuItem` when `ROW_KIND_FORM_REGISTRY[rowKind] === undefined`; or let the EXERCISE/FOOTNOTE/etc. steps naturally fill the registry. Minor.

- **QA-908 — `DEFAULT_STEPS` / `DEFAULT_REST_PARSED` are shared module-level mutable references** handed to RHF `defaultValues` (INFO; Step 9.2 QA — lowest priority). Safe today — RHF deep-clones `defaultValues`; `commitDraft` builds fresh arrays; the 9.1 `StandaloneLoadRowForm` does the same and has not bitten. **Action when triggered**: `Object.freeze` the constants or have `toFormData` return fresh copies — only if a future refactor reads a default outside RHF's clone boundary.

### Step 9.1 follow-ups

- **QA-307 — `toRestrictedReference` reference desync (HARD Step 9.3 prerequisite)** (WARNING; Step 9.1 QA, planner-traced 2026-05-22). `load-percentage-fields.tsx` `toRestrictedReference` collapses any non-`movement_family` reference (i.e. `self` **or** `other_exercise`) to `{ scope: "self" }`. Editing a `STANDALONE_LOAD` row whose load is `percentage` + `other_exercise` shows "self" in the reference dropdown while RHF form state still holds the original `other_exercise` reference — a display/state desync; any reference-`Select` interaction (even re-selecting the shown value) silently rewrites the row to `self`, discarding `targetExerciseId`. **Dormant in 9.1** — no `other_exercise` standalone-load row can exist (Exercise is admin-only, no platform read-path; the seed creates no SchemaRows). **Action — Step 9.3 (mandatory)**: when `PercentageReferenceEditor` gains the `other_exercise` scope (its 3rd), **delete `toRestrictedReference`** and render the true scope — the design § 5.6 widening is purely additive. The 9.3 thesis MUST carry this; do not ship 9.3's exercise picker while `toRestrictedReference` still collapses the scope. Effort: folded into 9.3.

- **QA-301 — emptied `percentage.value` saves a `0%` row (deferred-domain, QA-201-adjacent)** (re-assessed WARNING → deferred-domain; Step 9.1 QA, planner 2026-05-22). Clearing the `Percentage` field → `Number("") === 0`; `loadSchema.percentage.value` is `z.number().min(0).max(200)` → `0` is contract-valid → a `0%` standalone-load row saves silently (no field error, unlike the `positive()` `valueKg` fields where `0` is rejected). The 9.1 form correctly mirrors the contract (D-9.1-8) and invents no `> 0` bound — inventing one would invent a domain constraint the frozen contract (`analysis/` § 2.2 verbatim) does not carry, exactly what `[[coach-pov-first]]` forbids; and `0%` of 1RM (an empty bar) is a plausible coach warm-up value. **Whether the domain forbids `0%` is a planner/coach call** needing `analysis/source/` grounding or a Denys consultation — paired with QA-201 (param integer ceilings). **Action when triggered**: a ratified domain sub-step alongside QA-201 — if `0%` is disallowed, the fix is a contract `.refine` + `analysis/` sync, not a form patch. Not blocking; no corruption (one contract-valid value).

- **QA-303 — `movementFamily` accepts a whitespace-only string** (INFO; Step 9.1 QA — contract gap). `percentageReferenceSchema` `movement_family` requires `movementFamily: z.string().min(1)` with no `.trim()` → `"   "` (length 3) passes `min(1)`. 9.1 mirrors the contract exactly (D-9.1-11 forbids touching `packages/contracts`). **Action when triggered**: fold into a contract-hardening pass — `.trim()` or a `.regex(/\S/)` on `movementFamily`. Effort XS.

- **QA-305 — `SchemaRowCard` edit-form clobbered by a background week-query refetch** (INFO; Step 9.1 QA — codebase-wide, 8.4-shared). `SchemaRowCard`'s `editorMode = useMemo(() => ({ kind: "edit", row }), [row])` recomputes when the week query returns a freshly-parsed `row` object (a new reference even when byte-identical) → `StandaloneLoadRowForm`'s `useEffect(reset, [mode])` fires → in-progress edits to an open row-edit form are lost. **Identical to the shipped 8.4 `SchemaCard`** (`useMemo(..., [schema])`) — not a 9.1 regression; the R6 mandate (`useMemo` the mode) is satisfied, the residual is a property of the `useMemo(..., [entity])` pattern. `AddRowButton`'s create-mode form is unaffected (stable deps). **Action when triggered**: a plan-editor UX-polish pass — gate `reset` on the entity id (`mode.row.id`), not the row object, OR pause week-query refetch while an editor modal is open; fix `SchemaRowCard` + `SchemaCard` together. Effort M (codebase-wide).
...
- **QA-W2 — `lmsSchemaRowApi.reorder` array-form `$transaction([...])` cannot embed plan re-fetch** (WARNING). Array form, not interactive tx. Either convert к interactive `$transaction(async tx)` or accept the race as known-defer. **Action when triggered**: future `/fix` cycle, decide form at that time. **8.3.6 note**: the array-form reorder is confirmed two-pass + `@@unique([schemaId, order])`-compatible (intra-tx trace — `log/step-08.3.6.md` § 5 axis 3); 8.3.6 did not touch `schema-row/admin.ts` — the plan-recheck gap is unchanged, the WARNING stands.
