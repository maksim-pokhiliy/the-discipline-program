...
**Branch state**: `feat/training-domain` — recreated from fresh `main` (`be988162`) after **PR #202 merged** (Steps 8.3.7 + 8.4 — the `Schema` dual-scope uniqueness constraint + the coach-visible Schema editor anchor). **0 commits ahead**; this housekeeping commit records the merge + recreate (mirror `9220452a`). PR cadence: #199 = 8.1c+8.1d, #200 = 8.2+8.3, #201 = 8.3.5+8.3.6, #202 = 8.3.7+8.4; the next PR batches the post-#202 work once enough sub-steps accrue — the planner does not open it unprompted. All Step 8 infrastructure **and** the 8.4 anchor are shipped: the coach can create a plan, programme a block with schemas, pick from 34 archetypes, fill the `amrap-flat` / `n-rounds` param forms, and the schema cards render — bodies fill in the Step 9 row editor.
