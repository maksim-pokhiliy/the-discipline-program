...
**Branch state**: `feat/training-domain` — 13 commits ahead of `main` (6 Step-8.1c + 1 close-out + 6 Step-8.1d code/docs/test + 1 close-out = this commit). **Local, unpushed** — user pushes (ratified 2026-05-20). The api-server vertical for `AlternatingGroup` is complete; only HTTP wiring + read surface remain before the UI anchor.
