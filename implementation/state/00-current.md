...
**Branch state**: `feat/training-domain` — local, **9 commits ahead of `main`** (`e48c2b33`), unpushed, no PR open: PR #200 merge-housekeeping (`3193ddd3`), the 8.3.5 prompt (`c80e18c7`), the 8.3.5 squash (`2ee659cd`), the 8.3.5 output (`0688cc0a`), the 8.3.5 close-out (`88e76863`), the 8.3.6 prompt (`a42f17cf`), the 8.3.6 commit (`b32fd892`), the 8.3.6 output report (`d4afa166`), this close-out (`2c84ca0e`). PR cadence — the prior batch (Steps 8.2 + 8.3) merged as PR #200; the next PR batches the post-#200 work (8.3.5 + 8.3.6 + …) once enough sub-steps accrue. The write **and** read paths for the three slices are complete end-to-end; `SchemaRow` now also carries the DB-level positional-uniqueness constraint.
