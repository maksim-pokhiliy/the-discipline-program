...
**Branch state**: `feat/training-domain` — pushed to `origin`, **18 commits ahead of `main`** (`9a5c217e`). **PR #200 open against `main`** (created 2026-05-21) — batches Steps 8.2 (HTTP routes) + 8.3 (client hooks); the prior batch was PR #199 (Steps 8.1c + 8.1d, merged 2026-05-20). Awaiting review/merge — on merge the branch is typically re-cut from fresh `main` (precedent #195 / #198 / #199), so the next session must `git fetch` + check the PR state before resuming. The **write** path for the three slices is complete end-to-end (contracts → api-server → routes → client hooks); the read surface (`schemas[]` embed) remains before the UI anchor.
