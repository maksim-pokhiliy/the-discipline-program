...
**Branch state**: `feat/training-domain` — local, not pushed; **18 commits ahead of `main`** (`9a5c217e`) — the Step 8.2 close-out + Step 8.3 (prompt + 2 code + output + this close-out). PR #199 (Steps 8.1c + 8.1d) merged 2026-05-20; Steps 8.2 and 8.3 sit on the branch unpushed, no PR — per the workflow + user direction 2026-05-21: the branch stays local between steps, a PR batches when the training-domain epic closes (not per-step). The **write** path for the three slices is now complete end-to-end (contracts → api-server → routes → client hooks); the read surface (`schemas[]` embed) remains before the UI anchor.
