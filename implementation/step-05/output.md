...
2. Открыть любой ACTIVE-план (клик по карточке).
   ...
   недели, блок «Week notes», и **7 full-width строк** Mon-Sun **текущей календарной недели**;
   тело каждой строки — `"No sessions"`.
...
7. Нажать **Today**. _Ожидание:_ viewport вернулся на текущую календарную неделю;
   `?week=` = Monday текущей недели.
8. В поле **«Jump to date»** выбрать дату через 2 месяца. _Ожидание:_ viewport прыгнул на
   Monday той недели; `?week=` обновился. Очистить поле — no-op (поле снапается обратно).
