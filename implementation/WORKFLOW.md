...
Eight "read/verify-then-spec" procedural checklists accreted across Steps 1-7.3.6 (full per-step diagnoses in `IMPLEMENTATION_LOG.md` per-step "Open questions" + "Process note" footers):
...
None is optional. Eight together = procedural checklist; the collaboration rule ([[collaboration-co-ownership-claude-md]]) sets the stance.
