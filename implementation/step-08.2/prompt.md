...
2. **Widen the reorder request schemas** (D-8.2-4):
   - `reorderSchemasRequestSchema` — currently `= reorderSchemasSchema`. Redefine so it carries `orderedIds` **and** the discriminated scope (`blockId` xor `parentSchemaId`). The route handler splits this into the `CreateScope` + `ReorderSchemasData` the api wants. Exact Zod shape (discriminated union / object+`superRefine` enforcing exactly-one) — executor's call; mirror the `trailingConnectorSchema` `superRefine` idiom already in `schema.schema.ts` if going the refine route.
...
2. ✅ `reorderSchemasRequestSchema` carries the discriminated scope (`blockId` xor `parentSchemaId`) + `orderedIds`; `reorderSchemaRowsRequestSchema` carries `schemaId` + `orderedIds`; the entity-level `reorderSchemasSchema` / `reorderSchemaRowsSchema` are unchanged.
