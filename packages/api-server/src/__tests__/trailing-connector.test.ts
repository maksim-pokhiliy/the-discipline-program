...
describe("extractTrailingConnector", () => {
  ...
  it("MT-26: parses the canonical 'connector: then:' form (15 occurrences in plan-denys.json)", () => {
  ...
  it("MT-26: parses the canonical 'connector: ...then...:' form (2 occurrences in plan-denys.json)", () => {
    const result = extractTrailingConnector("connector: ...then...:");
  ...
  it("MT-26: parses the canonical 'connector: ...then N rounds:' form, capturing roundsCount", () => {
  ...
  it("matches connector hints case-insensitively (canonical ships both 'then' and 'THEN' spellings)", () => {
    const result = extractTrailingConnector("connector: ...THEN 2 rounds:");

    expect(result.connector).toEqual({ form: "then_n_rounds", roundsCount: 2 });
  });

  it("MT-30 (QA-004 regression): strips every connector line when two hints share the same notes string", () => {
  ...
  it("MT-30 (QA-005 regression): rejects roundsCount ≥ 100 by throwing at extract time", () => {
  ...
  it("MT-30 (QA-005 regression): regex cap of 1-2 digits means 100+ never reaches the build step", () => {
