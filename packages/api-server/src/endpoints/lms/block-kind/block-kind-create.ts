...
import { type CreateBlockKindInput } from "@repo/contracts/lms/block-kind";
...
export const createBlockKindImpl = async (userId: string, data: CreateBlockKindInput) => {
  ...
  if (scope === "SYSTEM") {
    ...
    if (!data.ownerId) {
      throw new BadRequestError(
        "Owner is required when ADMIN or HEAD_COACH creates a COACH-scoped block kind",
      );
    }
