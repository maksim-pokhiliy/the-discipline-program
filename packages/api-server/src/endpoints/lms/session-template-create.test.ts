...
describe("lmsSessionTemplateApi.create with ownerId (integration)", () => {
  ...
  const baseInput = (name: string) => ({
    ...
    name,
    payload: makeSessionPayload(fixture.blockKindId, fixture.exerciseId),
  });
  ...
  it("admin creating COACH-scope without ownerId rejects with 400", async () => {
    await expect(
      lmsSessionTemplateApi.create(
        admin.id,
        baseInput(`ST AdminNoOwner ${crypto.randomUUID().slice(0, 8)}`),
      ),
    ).rejects.toMatchObject({ statusCode: 400 });
  });

  it("admin creating COACH-scope with valid coach ownerId succeeds", async () => {
    const created = await lmsSessionTemplateApi.create(admin.id, {
      ...baseInput(`ST AdminCoachOwner ${crypto.randomUUID().slice(0, 8)}`),
      ownerId: fixture.coachUserId,
    });

    toCleanup.push({ table: "sessionTemplate", id: created.id });
  ...
  it("admin creating COACH-scope with non-existent ownerId rejects with 404", async () => {
    await expect(
      lmsSessionTemplateApi.create(admin.id, {
        ...baseInput(`ST AdminMissingOwner ${crypto.randomUUID().slice(0, 8)}`),
  ...
  it("admin creating COACH-scope with athlete ownerId rejects with 400 (not coach-like)", async () => {
    await expect(
      lmsSessionTemplateApi.create(admin.id, {
        ...baseInput(`ST AdminAthleteOwner ${crypto.randomUUID().slice(0, 8)}`),
        ownerId: athlete.id,
      }),
    ).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  it("coach passing ownerId in create payload rejects with 403", async () => {
    await expect(
      lmsSessionTemplateApi.create(fixture.coachUserId, {
        ...baseInput(`ST CoachWithOwnerPayload ${crypto.randomUUID().slice(0, 8)}`),
        ownerId: fixture.coachUserId,
      }),
  ...
  it("admin creating SYSTEM-scope ignores supplied ownerId and stores null", async () => {
    const created = await lmsSessionTemplateApi.create(admin.id, {
      ...baseInput(`ST AdminSystemIgnoreOwner ${crypto.randomUUID().slice(0, 8)}`),
      scope: "SYSTEM",
      ownerId: fixture.coachUserId,
    });
  ...
  it("coach creating SYSTEM-scope rejects with 403", async () => {
    await expect(
      lmsSessionTemplateApi.create(fixture.coachUserId, {
        ...baseInput(`ST CoachSystem ${crypto.randomUUID().slice(0, 8)}`),
        scope: "SYSTEM",
      }),
    ).rejects.toMatchObject({ statusCode: 403 });
  });
