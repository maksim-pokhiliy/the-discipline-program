...
import { type UpdateSessionTemplateInput } from "@repo/contracts/lms/session-template";
...
export const updateSessionTemplateImpl = async (
  userId: string,
  ...
  if (isPrivileged && (wantsScopeChange || wantsOwnerChange)) {
    if (targetScope === "SYSTEM") {
      ...
      if (!newOwnerId) {
        throw new BadRequestError("COACH-scoped session template requires ownerId");
      }
      ...
      if (collision) {
        throw new BadRequestError("Owner already has a session template with this name");
      }
  ...
  try {
    const item = await prisma.sessionTemplate.update({
