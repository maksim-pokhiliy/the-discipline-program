...
import { type CreateSessionTemplateInput } from "@repo/contracts/lms/session-template";
...
export const createSessionTemplateImpl = async (
  userId: string,
  data: CreateSessionTemplateInput,
) => {
  ...
  if (scope === "SYSTEM" && !isAdminPath) {
    throw new ForbiddenError("Only admin or head_coach can create SYSTEM session templates");
  }
  ...
  if (scope === "SYSTEM") {
    ...
    if (!data.ownerId) {
      throw new BadRequestError(
        "Owner is required when ADMIN or HEAD_COACH creates a COACH-scoped session template",
      );
    }
  ...
  try {
    const item = await prisma.sessionTemplate.create({
      data: {
