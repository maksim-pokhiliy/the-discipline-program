...
import { type CreateBlockTemplateInput } from "@repo/contracts/lms/block-template";
...
export const createBlockTemplateImpl = async (userId: string, data: CreateBlockTemplateInput) => {
  ...
  if (scope === "SYSTEM" && !isAdminPath) {
    throw new ForbiddenError("Only admin or head_coach can create SYSTEM block templates");
  }
  ...
  if (scope === "SYSTEM") {
    ...
    if (!data.ownerId) {
      throw new BadRequestError(
        "Owner is required when ADMIN or HEAD_COACH creates a COACH-scoped block template",
      );
    }
  ...
  try {
    const item = await prisma.blockTemplate.create({
      data: {
