...
describe("iamUserAdminApi", () => {
  ...
  describe("updateRole", () => {
    it("changes user role from USER to COACH", async () => {
      const updated = await iamUserAdminApi.updateRole(regularUser.id, {
        role: UserRole.COACH,
      });
      ...
      await iamUserAdminApi.updateRole(regularUser.id, { role: UserRole.USER });
    });

    it("throws NotFoundError for non-existent user", async () => {
      const fakeId = crypto.randomUUID();

      await expect(iamUserAdminApi.updateRole(fakeId, { role: UserRole.ADMIN })).rejects.toThrow(
        NotFoundError,
      );
    });
    ...
    it("throws ConflictError when demoting the last admin", async () => {
      ...
      for (const id of otherAdminIds) {
        await iamUserAdminApi.updateRole(id, { role: UserRole.USER });
      }

      try {
        await expect(
          iamUserAdminApi.updateRole(soloAdmin.id, { role: UserRole.USER }),
        ).rejects.toThrow(ConflictError);
      } finally {
        for (const id of otherAdminIds) {
          await iamUserAdminApi.updateRole(id, { role: UserRole.ADMIN });
        }
    ...
    it("allows demoting admin when other admins exist", async () => {
      ...
      try {
        const updated = await iamUserAdminApi.updateRole(adminUser.id, {
          role: UserRole.USER,
        });

        expect(updated.role).toBe(UserRole.USER);
      } finally {
        await iamUserAdminApi.updateRole(adminUser.id, { role: UserRole.ADMIN });
