...
import { type CanonicalSeed, type CanonicalSession } from "../_canonical/canonical-schema";
...
const createTrainingPlan = async (
  ...
  const description =
    seed.plan.description === null
      ? `Demo plan for ${seed.plan.athleteName}.`
...
export const seedCanonicalPlanShell = async (
  ...
  console.log(
    `  Demo Plan: planId=${demoPlanId}, ${sortedWeeks.length} sheet weeks + ${seed.phase7Examples.length > 0 ? 1 : 0} phase-7 week, ${sessionRefs.size} sessions`,
  );
