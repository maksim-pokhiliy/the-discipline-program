...
const ENTITY_INVARIANT_CELLS: readonly CoverageCell[] = [
  ...
  },
  {
    ...
    tally: (db, planId) => countSchema(db, planId, { notes: { not: Prisma.AnyNull } }),
  },
  {
    id: "entity.blockNotes",
    category: "entity-invariants",
    label: "Block with notes set",
    required: 1,
    sourceRef: "coverage-matrix §2 block-notes",
    tally: (db, planId) => countBlock(db, planId, { notes: { not: Prisma.AnyNull } }),
  },
  {
