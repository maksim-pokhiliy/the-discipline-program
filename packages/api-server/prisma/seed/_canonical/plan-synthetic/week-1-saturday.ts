...
const PRIMARY_PULL_UP_ROW_ID = "demo-wk1sat-parallel-pullup";
const PRIMARY_PUSH_UP_ROW_ID = "demo-wk1sat-parallel-pushup";
...
const BLOCK_PARALLEL_LADDERS_DESC_WK1_SAT: CanonicalBlock = {
  ...
  schemas: [
    parallelLaddersDescending({
      order: 1,
      header: "Demo parallel ladders 36-28-20 / 18-14-10",
      ladders: [
...
const BLOCK_LADDER_VERTEX_PYRAMID_WK1_SAT: CanonicalBlock = {
  ...
  schemas: [
    ladderVertexDownPyramid({
      order: 1,
      steps: [3, 6, 9, 6, 3],
      header: "Demo vertex pyramid 3-6-9-6-3",
      rows: [
        mkRow(
          1,
...
const BLOCK_URL_ONLY_BODY_WRAPPED_WK1_SAT: CanonicalBlock = {
  ...
  schemas: [
    urlOnlyBody({
      order: 1,
      header: "Demo URL only body (wrapped)",
      rows: [
        mkRow(
          1,
          {
...
const BLOCK_STRENGTH_ENDURANCE_EMPTY_WK1_SAT: CanonicalBlock = {
  ...
  intensity: null,
  timeCap: null,
  notes: "Demo STRENGTH ENDURANCE — empty body block",
  schemas: [],
};
...
const BLOCK_MULTILABEL_WK1_SAT: CanonicalBlock = {
  ...
  schemas: [
    ladderAscending({
      order: 1,
      steps: [3, 5, 7],
      header: "Demo Strength Endurance + Gymnastics ladder",
      rows: [
        mkRow(
          1,
