...
- **No inline sizing overrides on MUI components.** Never override `fontSize`, `padding`, `minHeight` via `sx` to make a component bigger/smaller — use MUI's `size` prop. If no suitable size exists, add a new size variant to the theme (e.g. `sizeXlarge`). Per-instance sizing kills consistency and creates N different "large" buttons across the app.
