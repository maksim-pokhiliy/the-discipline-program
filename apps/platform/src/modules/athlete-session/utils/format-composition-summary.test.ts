...
import {
  ...
  formatRestSummary,
} from "./format-composition-summary";
...
describe("single-part accessors used by the schema card", () => {
  it("formatRepetitionLabel returns the repetition badge text", () => {
    const composition: Composition = {
      repetition: { kind: "timeCap", cap: { min: 20, unit: "min" } },
    };

    expect(formatRepetitionLabel(composition)).toBe("cap 20’");
  });

  it("formatRepetitionLabel returns null when no repetition axis is set", () => {
    expect(formatRepetitionLabel({})).toBeNull();
  });

  it("formatCapSummary returns the cross-cutting cap text", () => {
    const composition: Composition = { cap: { min: 8, unit: "min" } };

    expect(formatCapSummary(composition)).toBe("cap 8’");
  });

  it("formatRestSummary returns the rest text", () => {
    const composition: Composition = {
      rest: { duration: { value: 2, unit: "min" }, scope: "between_sets" },
    };

    expect(formatRestSummary(composition)).toBe("rest 2 min between sets");
  });
});
