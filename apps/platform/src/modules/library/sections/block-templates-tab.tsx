...
export const BlockTemplatesTab = ({ currentUserId }: BlockTemplatesTabProps) => {
  ...
  const isOwn = useCallback(
    (item: BlockTemplate) => item.scope === "COACH" && item.ownerId === currentUserId,
    [currentUserId],
  );

  const filters: DataTableFilter<BlockTemplate>[] = useMemo(
    () => [
      {
        id: "scope",
        label: "Scope",
  ...
  return (
    <Stack spacing={2}>
      ...
      <DataTable
        ...
        columns={columns}
        filters={filters}
        searchPlaceholder="Search block templates..."
        paginated
        emptyMessage={
          isLoading ? "Loading block templates..." : "No block templates match your filters."
        }
        state={state}
