...
import {
  ...
  useDemoteWeekTemplate,
  usePlatformCoaches,
  usePromoteWeekTemplate,
...
export const WeekTemplatesTab = ({ currentUserId }: WeekTemplatesTabProps) => {
  const role = useCurrentUserRole();
  const isPrivileged = role === UserRole.HEAD_COACH || role === UserRole.ADMIN;

  const [demoteTarget, setDemoteTarget] = useState<WeekTemplate | null>(null);
  const [editTarget, setEditTarget] = useState<WeekTemplate | null>(null);

  const { data, isLoading, error } = useWeekTemplatesPageData({}, currentUserId);
  const { data: coaches } = usePlatformCoaches(isPrivileged);

  const deleteMutation = useDeleteWeekTemplate();
  ...
  const isOwn = useCallback(
    (item: WeekTemplate) => item.scope === "COACH" && item.ownerId === currentUserId,
    [currentUserId],
  );

  const filters: DataTableFilter<WeekTemplate>[] = useMemo(
    () => [
      {
        id: "scope",
        label: "Scope",
  ...
  const columns: Column<WeekTemplate>[] = useMemo(
    () => [
      {
        id: "name",
        label: "Name",
  ...
  return (
    <Stack spacing={2}>
      ...
      <DataTable
        ...
        columns={columns}
        filters={filters}
        searchPlaceholder="Search week templates..."
        paginated
        emptyMessage={
          isLoading ? "Loading week templates..." : "No week templates match your filters."
        }
        state={state}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this week template?"
