...
const PASSIVE_ARM_ACTION_LABELS: Record<string, string> = {
...
const formatWeight = (weight: Weight): string => {
  switch (weight.variant) {
    ...
    case "compound_device":
      return `${weight.valueKg} kg · ${weight.count}× ${EQUIPMENT_SHORT[weight.equipment] ?? weight.equipment}`;
    case "split_tier":
      return `split: ${weight.stages
        .map(
          (stage) =>
            `${stage.reps}×${stage.valueKg} kg ${EQUIPMENT_SHORT[stage.equipment] ?? stage.equipment}`,
        )
        .join(", ")}`;
    case "dual_value":
      return `${weight.first} / ${weight.second} kg`;
    case "with_asymmetric_arm": {
      const arm = ARM_LABELS[weight.workingArm] ?? weight.workingArm;
      const action = PASSIVE_ARM_ACTION_LABELS[weight.passiveArmAction] ?? weight.passiveArmAction;
      const extra =
    ...
    case "with_depth_modifier":
      return `${weight.valueKg} kg ${DEPTH_LABELS[weight.depth] ?? weight.depth}`;
    default:
      return "weight";
  }
};
