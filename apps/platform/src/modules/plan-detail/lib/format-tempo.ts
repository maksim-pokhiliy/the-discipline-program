...
export const formatTempo = (tempo: TempoModifier): string =>
  `${TEMPO_PREFIX}${digit(tempo.eccentric)}${TEMPO_SEPARATOR}${digit(tempo.pauseBottom)}${TEMPO_SEPARATOR}${digit(tempo.concentric)}${TEMPO_SEPARATOR}${digit(tempo.pauseTop)}`;
