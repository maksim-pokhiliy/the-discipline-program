...
const BASE = "/api/admin/library/exercises";
...
export const createLibraryExercisesAPI = (client: ApiClient) => ({
  ...
  approve: (id: string): Promise<Exercise> => client.request(`${BASE}/${id}/approve`, "POST"),

  reject: (id: string, data: RejectExerciseData): Promise<Exercise> =>
    client.request(`${BASE}/${id}/reject`, "POST", data),

  merge: (id: string, data: MergeExerciseData): Promise<Exercise> =>
    client.request(`${BASE}/${id}/merge`, "POST", data),
});
