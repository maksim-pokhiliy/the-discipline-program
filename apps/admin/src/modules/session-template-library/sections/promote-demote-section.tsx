...
import { useDemoteSessionTemplate, usePromoteSessionTemplate } from "@app/lib/hooks";
...
export const PromoteDemoteSection = ({
  ...
  const handlePromote = () => {
    ...
    promoteMutation.mutate(promoteTarget.sessionTemplate.id, { onSuccess: onClose });
  };
  ...
  const handleDemote = () => {
    ...
    demoteMutation.mutate(
      { id: demoteTarget.sessionTemplate.id, data: { newOwnerId } },
      { onSuccess: onClose },
    );
  };
  ...
  return (
    <>
      <ConfirmationModal
        ...
        message={promoteTarget ? `Promote '${promoteTarget.sessionTemplate.name}' to SYSTEM?` : ""}
      ...
      >
        <Stack spacing={2}>
          <Typography variant="body1">
            {demoteTarget
              ? `Demote '${demoteTarget.sessionTemplate.name}' to a coach-owned session template?`
              : ""}
          </Typography>

          <Typography variant="body2" color="text.secondary">
            The session template will be reassigned to the selected coach and removed from the
            SYSTEM library.
          </Typography>
