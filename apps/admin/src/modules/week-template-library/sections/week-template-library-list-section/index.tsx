...
type WeekTemplateLibraryListSectionProps = {
  items: WeekTemplate[];
};

export const WeekTemplateLibraryListSection = ({ items }: WeekTemplateLibraryListSectionProps) => {
  ...
  const handlePromoteRequest = useCallback((weekTemplate: WeekTemplate) => {
    setPromoteTarget({ weekTemplate });
  }, []);

  const handleDemoteRequest = useCallback((weekTemplate: WeekTemplate) => {
    setDemoteTarget({ weekTemplate });
  }, []);
        ...
        render: (item) => (
          <Stack direction="row" spacing={0.5} justifyContent="flex-end">
            ...
            <Tooltip title="Edit">
              <IconButton
                component={Link}
                href={`/library/week-templates/${item.id}`}
                color="primary"
                aria-label="Edit"
                size="small"
              >
  ...
  return (
    <>
      <DataTable
        ...
        filters={filters}
        paginated
        emptyMessage="No week templates yet."
        state={state}
