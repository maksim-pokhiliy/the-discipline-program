...
import { useDemoteBlockTemplate, usePromoteBlockTemplate } from "@app/lib/hooks";
...
export const PromoteDemoteSection = ({
  ...
  const promoteMutation = usePromoteBlockTemplate();
  const demoteMutation = useDemoteBlockTemplate();
  ...
  const handlePromote = () => {
    ...
    promoteMutation.mutate(promoteTarget.blockTemplate.id, { onSuccess: onClose });
  };
  ...
  const handleDemote = () => {
    ...
    demoteMutation.mutate(
      { id: demoteTarget.blockTemplate.id, data: { newOwnerId } },
      { onSuccess: onClose },
    );
  };
  ...
  return (
    <>
      <ConfirmationModal
        ...
        message={promoteTarget ? `Promote '${promoteTarget.blockTemplate.name}' to SYSTEM?` : ""}
      ...
      >
        <Stack spacing={2}>
          <Typography variant="body1">
            {demoteTarget
              ? `Demote '${demoteTarget.blockTemplate.name}' to a coach-owned block template?`
              : ""}
          </Typography>

          <Typography variant="body2" color="text.secondary">
            The block template will be reassigned to the selected coach and removed from the SYSTEM
            library.
          </Typography>
