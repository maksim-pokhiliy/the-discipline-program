...
type PromoteState = { blockTemplate: BlockTemplate } | null;
type DemoteState = { blockTemplate: BlockTemplate } | null;

type BlockTemplateLibraryListSectionProps = {
  items: BlockTemplate[];
};

export const BlockTemplateLibraryListSection = ({
  items,
}: BlockTemplateLibraryListSectionProps) => {
  ...
  const handlePromoteRequest = useCallback((blockTemplate: BlockTemplate) => {
    setPromoteTarget({ blockTemplate });
  }, []);

  const handleDemoteRequest = useCallback((blockTemplate: BlockTemplate) => {
    setDemoteTarget({ blockTemplate });
  }, []);
  ...
  const columns: Column<BlockTemplate>[] = useMemo(
    () => [
      {
        id: "name",
        label: "Name",
        ...
        render: (item) => (
          <Stack direction="row" spacing={0.5} justifyContent="flex-end">
            ...
            <Tooltip title="Edit">
              <IconButton
                component={Link}
                href={`/library/block-templates/${item.id}`}
                color="primary"
                aria-label="Edit"
                size="small"
              >
  ...
  return (
    <>
      <DataTable
        ...
        filters={filters}
        paginated
        emptyMessage="No block templates yet."
        state={state}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this block template?"
