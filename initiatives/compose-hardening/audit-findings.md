...
- **T1-3 — depth-2 projector truncation (CARRY `QA-106`).** `minor` (latent). `buildSchemaWithBody` hard-codes `subSchemas:[]` at level 2 (`schema.mapper.ts:38-42`); both the write-guard and the read-path `assertComposeTreeValid` reuse it → a ladder/marker collision at depth-3 evades both silently. Safe while nesting is shallow; a guard or explicit depth-≤2 assert is owed before nesting deepens.
  - **[CORRECTED 2026-06-07 — `major`, NOT `minor`/latent. Fixed in `feat/compose-tier1-correctness`; see `deferred.md` T1-3.]** Re-verify on live code found a **seeded repro**: `block-010` (`BLOCK_NESTED_OVER_PARALLEL_LADDER_WK2_TUE`, `week-2-tuesday-compose.ts:125-202`) is depth-3 (rounds(2) → parallel → 2 ladder tracks), in the **active** seed (`week-2-tuesday.ts:29`) and recursively persisted (`schema-emit.ts:61-88`). The sharper edge is **not** "collision evades the write-guard" (the write-guards re-root per write, so the local ladder-collision `superRefine` is always caught — they were sound) — it is **silent read-side data-loss**: the read truncates at L2, so block-010's two ladder tracks (its entire workout) vanish on the coach's week view with no error. Fix = recursive read (`buildSchemaForest`, flat-fetch + in-memory rebuild); the depth-≤2 cap option was **rejected** (would contradict the seeded block-010 + the uncapped free-nesting design + the recursive `SchemaWithBody` contract). Bonus: read-side `assertComposeTreeValid` now runs full-depth.
