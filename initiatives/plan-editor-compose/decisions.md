...
| D-BYPROFILE-AXIS   | byProfile axis → discriminated union (cross-ref: profile-axis-catalog W2 four-projection re-check)  | RATIFIED (cross-ref 2026-06-22)              |
...
- **Status:** RATIFIED (cross-ref, 2026-06-22). Recorded HERE because the `byProfile` load is a `SchemaRow` Json-VO on this initiative's "Sacred (do not touch)" list — a change to it requires a four-projection re-check + a decision in this file (the gate `profile-axis-catalog/decisions.md` D-3 named). Full analysis: `initiatives/profile-axis-catalog/four-projection-recheck.md`.
- **Change.** The `byProfile` load axis `{ name:string, values:string[] }` (free-string, frozen by primitive-v2 #17) becomes `{ kind:"catalog", axisId, label, values } | { kind:"human", attribute:"gender" }`. cells (coords/kg) + the 1–2 axes cap are PRESERVED; the change is the axis identity only.
