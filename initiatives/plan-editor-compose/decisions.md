...
| D-BYPROFILE-AXIS   | byProfile axis → `binding`-bound catalog axis (design A; cross-ref: profile-axis-catalog W2/D-7)    | RATIFIED (cross-ref; rev 2026-06-23)         |
...
### D-BYPROFILE-AXIS — byProfile axis → discriminated union (cross-ref from profile-axis-catalog W2)

- **Status:** RATIFIED (cross-ref, 2026-06-22; **revised 2026-06-23 → design A**). Recorded HERE because the `byProfile` load is a `SchemaRow` Json-VO on this initiative's "Sacred (do not touch)" list — a change to it requires a four-projection re-check + a decision in this file (the gate `profile-axis-catalog/decisions.md` D-3/D-7 named). Full analysis: `initiatives/profile-axis-catalog/four-projection-recheck.md` (Part B = design A).
- **Revision (design A, 2026-06-23 — supersedes the union in the bullets below).** Owner smoke exposed the union's hole: the `{kind:"human"}` arm was bypassable by a coach-made `{kind:"catalog"}` axis named "Gender", which resolved from a manual pick and ignored the typed `gender` column — TWO authoring paths for gender (the SET-projection collision the union re-check missed). Design A replaces the union with a SINGLE `{ axisId, label, values, binding }` object discriminated by `binding` (`null` = plain pick | `GENDER` = bound to the typed column). gender = one SYSTEM, PROTECTED catalog row (`binding=GENDER`, `@@unique([binding])`, not API-settable) — authorable exactly once, a second impossible by construction. Sacred-VO sign-off RE-AFFIRMED: four-projection PASSED on the new shape (bound vs plain each four-projection-invariant, now incl. SINGLE-PATH authoring). Owner of the change: profile-axis-catalog (D-7).
- **Change (SUPERSEDED by design A above — retained for history).** The `byProfile` load axis `{ name:string, values:string[] }` (free-string, frozen by primitive-v2 #17) becomes `{ kind:"catalog", axisId, label, values } | { kind:"human", attribute:"gender" }`. cells (coords/kg) + the 1–2 axes cap are PRESERVED; the change is the axis identity only.
- **Four-projection verdict.** PASSED — and it is the lens that DEMANDS the change: the free-string axis is a latent name-collision (`name:"gender"` resolves from the typed `AthleteProfile.gender` column / groups by intrinsic identity, vs `name:"level"` resolves from a manual pick / groups by mutable classification — "depends what was meant" in EXECUTE + ANALYTICS). The discriminated union is the mandated split on `kind`, structurally identical to `D-LADDER`. Each arm is four-projection-invariant; the `kind` discriminant + disjoint identity-space clear the cross-arm collision. Owner of the change: profile-axis-catalog (D-3); this file owns the sacred-VO sign-off.
- **Links.** `[[D-PERSIST]]` (the sacred VO), `[[compose-four-projection]]`; profile-axis-catalog D-3 + `four-projection-recheck.md`; primitive-v2 `D-V2-PROFILE-NESTING` (#17, superseded forward).
...
### D-10.4-S3 — S3 execution: discover-corrected split (A-B done / C-E next), Phase C is `/feature` not Workflow

- **Status:** RATIFIED (2026-06-04). S3 launched as an ultracode `Workflow` discover fan-out (6 read-only agents re-verifying every removal site vs current post-S2 code). A+B (the platform UI half) shipped this session on `feat/compose-s3` (`0fe3e6c3` render-flip + `25c7080f` authoring delete; platform 982 green; api-server/contracts untouched). C-E paused at the owner's pre-sanctioned split-point (`charter`/plan "фазы 1-2 vs 3-5").
- **Discover corrected the recon framing (load-bearing — the recon's "mechanical sweep" was wrong in two places):** (1) the card render-flip was UNDONE work, not done — A performed it (cards were still archetype-first; `grep deriveCompositionLabel apps/platform = 0` pre-A). (2) the seed (Phase C) is **64-node domain re-authoring** (~20 structural), the single largest task — NOT mechanical. (3) a full dead alt-group subsystem (4 routes + 4 unused mutation hooks) existed in platform — B removed it. (4) the coverage gate is ~2× what recon listed (7+3 extra archetype tests, a SEPARATE local `ARCHETYPE_NAMES`, `schemaKindCell`). (5) `block.mapper.ts`/`day.mapper.ts` embed `alternatingGroups` → break in the Phase D fold (rewire, not delete). The arrangement back-patch re-key (refId→cuid, two-phase because `childSchemaId` points at a child created after the parent) is the Phase-C crux.
- **Vehicle reversal (the contentful call):** the kickoff said "S3 mechanical → Workflow, not `/feature`." Discover SPLIT that: D + E ARE mechanical (contract/api-server removal + Prisma + gated run) → Workflow/direct holds; **C is domain authoring → `/feature` full** (the conversion mapping, structural nodes, back-patch — `/feature`'s review/qa gates de-risk it). The original premise (whole sweep mechanical) was falsified for C, so the vehicle reverts to the project's `/feature` convention ([[always-via-feature-skill]]) for that phase. Full Phase-C spec promoted to `phase-c-seed-conversion.md`; the archetype→composition mapping is grounded in `algebra-spec.md` §2.5/§3 (not invented).
- **A+B implementation calls.** Render-flip sources card label/header/meta from `deriveCompositionLabel` + `formatCompositionSummary` (both contract-native, already existed) + a slimmed `formatSchemaHeader(schema)`; `schema-archetype-tag` → renamed `schema-composition-tag` (reuse the chip, post-pivot name). The per-schema EDIT button removed (compose is create-only, S2 reality). Alt-group block-render → flat list; parallel/superset is surfaced via the composition label/summary, **the AccentGroupCard accent for parallel containers is intentionally NOT rebuilt** (no parallel data in the DB until the Phase-E reseed → speculative un-verifiable render; migration-philosophy no-preventive-polish — carry-forward as an optional later enhancement). REVIEW-005: only the 2 const label-maps are cleanly shareable (extracted to `lib/compose-axis-labels.ts`); full dedup is blocked by `compose-tree.types` diverging from the contract (range-folded-into-count) — residual deferred (`deferred.md`).
- **`trailingConnector` drop adjudicated.** One discover agent (seed-builder) dissented "not archetype-coupled → leave it"; OVERRIDDEN — the plan (RATIFIED 10.2: "folds into `arrangement:ordered`/presentation; no carry") + the owner's own phase-5 Prisma-drop list both drop it; it is the connector-form presentation layer the pivot subsumes ([[D-PHASE5-SCORING]]). Dropped in C (emit + 2 coverage cells) + D (contract field) + E (Prisma column).
- **Links:** `phase-c-seed-conversion.md` (Phase-C spec), `10-4-recon.md` (the now-partially-superseded recon — trust the discover corrections over it for C), [[D-D4-REVERSAL]], [[D-10.4-2]], [[D-ALTGROUP-FOLD]], [[D-LABEL]].

### D-10.4-S3-C — Phase C execution calls (seed composition-native; committed `ff8b0392`)

- **Status:** RATIFIED (2026-06-05; `feat/compose-s3` `ff8b0392`). Distilled from the `/feature` run's `.feature-dev/1780601689/{design,review,qa}.md` (gitignored). api-server check-types + lint + dep:check green = the Phase-C ceiling; the seed's RUNTIME validation is Phase E. 63 call-sites across **15** plan-synthetic files converted (the spec's "18/64" over-counted).
- **DR-C1 — aggressive no-bridge** ([[aggressive-migration-no-bridge]]): NO green-per-commit transitional bridge — direct flip, intermediate tree globally check-types-RED, only the integrated end-state green. The husky cone is pre-push (push at S3-done) so intermediate commits aren't gated. INFRA agent (engine) → 7 parallel CONVERT agents (file-disjoint) + 1 COVERAGE agent → orchestrator integrated + verified + committed.
- **DR-C2 — program/slot DEFERRED (DEFER-001).** The frozen contract + Prisma + S2 UI have NO row-level home for `StagedProgram`/`SlotSpec` (consumed only by `archetype-params.schema.ts`; no SchemaRow column; platform `ComposeProgram` = tree-of-weeks naming collision). **algebra §1's "program is a sacred Row-VO" was WRONG — it was always an archetype param.** The 5 named-program + emom-slot nodes → flat `{}` (exercise rows survive; staged-program + the 1 grouped-slot span deferred to D/E; single-minute slots lossless via D-EMOM-UX). Dropped the 5 `stagedProgram.*`/`slotSpec.*` coverage cells. Adding the row field = contract(`schema-row`)+Prisma = forbidden in C by the owner's phase boundaries.
- **ALT-1 — back-patch refIds INSIDE `composition.arrangement` (cuid-format via `cuidFromSeed`), NOT a `deferredArrangement` side-field.** `z.string().cuid()` infers `string` → type-checks; cuid-format passes the Phase-E `canonicalSeedSchema.parse`; mirrors the existing super-set `schemaRows` pattern (less machinery than the platform's branded-NodeId side-channel). The seed-side two-phase (`backPatchComposition`/`resolveArrangement` + a NET-NEW child-schema ref map in `ref-resolver`; strip-arrangement-at-create → register children → whole-composition patch) is the analog of S2-R2's platform persist. QA empirically ran `canonicalSeedSchema.safeParse(SYNTHETIC_DEMO_PLAN)` → **PARSE OK**.
- **DR-C3 — coverage gate SCOPED to emitted kinds** (14 per-axis cells; un-emitted once/range/prescribed/for_time documented, NO cell — [[no-list-caps-honest-counts]]; growing the seed with synthetic non-Gauntlet nodes = the corpus-measuring anti-pattern algebra §0 rejects). Re-pointed `entity.alternatingGroup` → `arrangement:parallel` presence. `buildComposeNode` moved into `builder/compose/` (zero archetype paths in the seed, DoD #4).
