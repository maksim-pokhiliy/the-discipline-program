...
| 10.4 | **Destructive sweep + QA-001 write-guard** — the arc (S1→S2→S3) below                                               | per-sub-step (see below); blocked on D-10.4-1/2/3 ratification            | 🔵 PLANNING (recon done → `10-4-recon.md`)                              |
...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | ---------------------------------------------------------------------------------------- | --------------------------- |
| S1  | QA-001 write-guard (400-sibling + 3 hooks) + QA-003 fold + nullable-archetype expand                                                                             | `/feature small`       | gated DB run; the read-500 test flips to write-400                                       | ⏳ next (after D-10.4-1/2)  |
| S2  | Productionize the compose-write prototype: draft→`Composition` converter + persistence + type-align + mounts + validation/QA-001 client feedback                 | `/feature` full        | **coach re-walkthrough** (validated UX survives the real backend + persisted round-trip) | ⛔ blocked on S1 + D-10.4-3 |
| S3  | Mechanical sweep: delete old authoring → render-flip → seed composition-native + gate-rewrite → contract/api-server archetype removal → Prisma drop + `db:reset` | **ultracode workflow** | husky cone green per commit; gated full suite after `db:reset`                           | ⛔ blocked on S2            |
...
- Still OPEN (10.4): D-10.4-1 (arc), D-10.4-2 (drop `kind`), D-10.4-3 (S2 scope).
