...
| 10.4 | **Destructive sweep + QA-001 write-guard** — the arc (S1→S2→S3) below                                               | per-sub-step (see below); D-10.4-1/2 RATIFIED, D-10.4-3 → S2 gate         | 🔵 IN PROGRESS — S1 done (PR #241); S2 next                             |
...
| S2  | Productionize the prototype: converter + persistence + **read-shape widen + composition-only handler enable (DR-1)** + type-align + mounts + QA-001 client feedback | `/feature` full        | **coach re-walkthrough** (validated UX survives the real backend + persisted round-trip) | ⏳ next (ratify D-10.4-3 at kickoff) |
...
- Still OPEN (10.4): D-10.4-3 (S2 scope) — leaning recorded, ratify at S2 kickoff. RATIFIED in S1: D-10.4-1 (arc), D-10.4-2 (drop `kind` direction), D-10.4-S1-RS (read-shape option b), D-10.4-S1-INT (poison-intermediate).
