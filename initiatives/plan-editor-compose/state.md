...
**Core arc 10.0–10.4 DONE + MERGED to `main` (`c20c082a`, PR #245, 2026-06-05).** The compose-only model is fully shipped — archetype excised across every layer (UI A/B, seed C, contract+api-server D, Prisma E; picker/archetype gone everywhere); the gated api-server suite ran **726 green**. `feat/compose-s3` is merged + deleted. **This initiative's build arc is concluded — do NOT resume it as in-progress.**

**Reconciliation DONE (2026-06-05) — `theory-vs-code-reconciliation.md`.** Verdict: code **faithful, ZERO unintentional drift**; every divergence from the origin web-Claude theory is ratified + documented. Unbuilt = gaps not debt: ph.5 scoring-execution (declared) + a newly-surfaced second-layer gap (**conditional-scoring has no authoring UI** though the contract froze the field) + fixture-incompleteness (seed exercises 6/8 repetition, 4/6 scoring, 0 stagedProgram-stages, 0 INNER_LADDER_MARKER; Gauntlet Block C never assembled end-to-end). Method: theory read verbatim + 5 parallel code-readers + self-verify (verify-not-trust caught 2 reader over-claims). 2 live lost-in-scratch INFO tails recovered → `deferred.md` (QA-106 depth-2 guard, QA-108 no-DB-backstop); QA-107/105 verified moot. ph.5 seed → artifact + memory [[compose-ph5-seed]].

**Next session = UX-polish round of the compose constructor** (owner call 2026-06-05 — «погулять по конструктору и подоёбываться к UI/UX»). Coach-daily-UX is the #1 project priority ([[coach-daily-ux-priority]]); the constructor is the authoring surface. **Exploratory polish, NOT build-from-spec** — owner walks the live constructor (`/coach/compose-prototype`) and snags; fixes triaged as found. Start-prompt handed to the owner.

**Seeded UX snag-list (from the reconciliation — VERIFY each live, don't trust blind):**

1. EMOM `MIN 1/2/3` labels NOT shown on the compose canvas — `deriveMinuteView` works but is wired only to `schema-row-list.tsx` (read-side), not the prototype canvas (`compose-container-card.tsx` renders only `formatAxesSummary`). Cheapest + most visible.
2. Ladder-collision error fires LATE — on Save (`compositionSchema.safeParse` in `convertContainer`), not at authoring time. Should surface on input.
3. Axis fields silently store malformed input (`everyMin=0`, `workMin=-5`, `25:99`, `rangeMax<value`) — `axis-fields-safety.test.tsx` pins this; validation lives at persist (contract-zod), no field-level UI validation.
4. `shouldBeContainer` shows a demote HINT (`compose-container-inspector.tsx:85` Alert) but no demote BUTTON — advice without action (button deferred, D-10.4-S2).
5. (feature-ish, not polish) No authoring input for conditional-scoring (`condition.appliesToRounds`) — contract froze the field, UI grep = NO MATCH.

**Live carry-forwards (non-blocking, `deferred.md`):** DEFER-001 (row-level program/slot → later phase) · ADR-0023 (`fast-check` reconcile) · cosmetic-stale. The `scoring` axis is present-but-inert (tripwire `scoring-inert-consumers.test.ts`). Vercel not configured ([[project-vercel-not-configured]]); `mirror` remote user-managed, origin-based ([[mirror-remote]]).
