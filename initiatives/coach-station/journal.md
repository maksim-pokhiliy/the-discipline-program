...
- **D-5 → D-7 (owner-amended).** Owner overrode D-5's "schema NOT expanded": the schema IS expanded (additive `CoachCredential` + `CoachProfile.location`/`specialties`). Ratified **D-7 PROFILE-SCHEMA-EXTENDED** (this PR, per `closeout-before-pr`); charter non-goal amended.
- **Shipped (10 commits, `feat/coach-profile`):** `ec72d05c` prisma+contracts · `038d9d0d` api-server (credential CRUD + profile page-data/two-table) · `1b840b8f` gated api-server tests · `59f73e15` platform routes+client+hooks+upload · `21fea0bc` lift `TimezoneAutocomplete`→`@repo/ui` · `950d30e8` extract `detectBrowserTimezone`→`@repo/shared` · `b7cded5d` UI (5 sections) · `55e3c8c6` review/QA fixes · `491239a6` contract+jsdom tests · `3104742b` blob-env docs.
- **Pipeline.** research (RE-VERIFY barrels / upload-absence / primitive props vs main) → design+plan (Gate A: ~30 tasks / 5 phases / ~41 files, ONE wave — a server-only split would hide the UI from a solo owner) → implement (5 phases) → Review **A** (0 crit/0 warn) / QA **B** (0 crit/3 warn) → fixed 5 (least-privilege upload, server-authoritative coachProfileId, dead-code drop, name maxLength, dedupe specialties) → tests (contracts 723 + platform 729 green; gated written-not-run).
- **UI via `ui-ux-pro-max`** (initiative rule): 5 sections (identity hero · track-record = 3 honest derived + DERIVED pill · credentials list+sheet+hard-delete-confirm · workspace tz+email), dark MUI theme consumed (no invented tokens), reused `ConfirmationModal`, circular avatar over a hidden file input (ImageUpload can't be circular).
- **Track-record = honest:** monthsActive (User.createdAt), athletesCoached (assignment count), plansAuthored (non-deleted). "Sessions delivered" / integrations / notifications explicitly OUT (façade-over-absent-infra).
- **Owner-owed (P-GATED-ACCEPTANCE):** `pnpm db:reset && pnpm --filter @repo/api-server test` (new suites) + browser walkthrough; **set `BLOB_READ_WRITE_TOKEN` in `apps/platform/.env.local`** for avatar upload. Deferred: QA-006 (per-row mutation state), QA-007 (tz-nudge UTC edge) → `deferred.md`.
- **Next.** R1b (main-line) · then G / A-known / R2-slot.
