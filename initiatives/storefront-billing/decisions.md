...
### D-3 — Recurring mechanism

- **Status:** OPEN — decided by the W0 spike (plan 0.2). Do not spec P1.3 past it.
- **Fork.** (a) Mono native subscriptions (`POST /api/merchant/subscription/create` → Mono schedules the charges; `edit` supports only `cancel` with an optional refund; `remove` only before the first payment; no documented retry semantics; fiscal-basket support unknown). (b) Own scheduler: first payment tokenizes the card (`saveCardData`), each period the platform cron issues `POST /api/merchant/wallet/payment` with `initiationKind: merchant`; every charge is an ordinary invoice (fiscalizable, carries `merchantPaymInfo`), retries / grace / 4-week math are ours.
- **Lean.** (b), unless the spike shows native subscriptions carry a fiscal basket AND expose retry semantics AND run in test mode. Criteria to record in `monobank-notes.md`: nested `webHookUrls`, the subscription status enum, the payload of a recurring-charge webhook, behaviour on a declined renewal, test-mode support, whether the client can cancel from the mono app.
- **Links.** `monobank-notes.md` "open items"; deferred SB-1.
