...
**Updated:** 2026-09-25 (later) — **0.2 spike run against the test merchant: the (b) loop is proven (tokenize → merchant-initiated charge ×3, synchronous success), native `4w` subscriptions also work in test mode but expose no retry semantics and no basket; D-3 PROPOSED (b), P0.3 next; webhooks wait on the SB-14 tunnel.** Earlier the same day: **P0.0 done: the local Docker stack is the target of every dev server, test run and migration rehearsal (D-14).** `docker-compose.yml` (`postgres:17-alpine`, `tdp` / `tdp_test` / `tdp_shadow`), `task stack:*` + `task test:api`, `docs/runbooks/local-stack.md`; the owner's env files now point at it with the Neon dev lines commented. Gate passed: athlete / coach / admin sign in on localhost, iOS shim sign-in 200, api-server suite green in 2 min 32 s against `tdp_test`. Two findings: prod parity wants Postgres 17 (CI stays 16, SB-15); Cloudflare quick tunnels are unreachable from the owner's network, so Mono webhooks in 0.2 need a named tunnel on the zone in Vladyslav's account (SB-14, owner's one-time login).
...
| --- | --------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0  | Foundation      | 🔵 active  | 0.0 ✅ · 0.1 ✅ · 0.2 🟡 (D-3 proposed, webhooks wait on SB-14) · **0.3 migration next** · 0.3 migration · 0.4 `uk` pages · 0.5 env/port/adapter · target 02.10 |
...
**▶ Owner: ratify D-3 = (b) (own scheduler + merchant-initiated charges on the tokenized card; evidence in `monobank-notes.md` §0.2 spike). Then planner: the 0.3 W0 migration corpus + executor prompt from `domain-model.md` (planner-discipline (h) trace on the new uniques before locking it); 0.4 and 0.5 run as parallel executors once 0.3's schema is merged. Webhook capture is a 30-minute session once the SB-14 named tunnel exists: `node initiatives/storefront-billing/spike/mono-spike.mjs webhook:listen` on 3999, tunnel → 3999, one invoice with `--webhook`, one merchant-initiated charge with `--webhook`, one subscription with `--webhook` (`webHookUrls` field names are a guess until then); the captured invoice webhook becomes the 0.5 signature fixture.**
