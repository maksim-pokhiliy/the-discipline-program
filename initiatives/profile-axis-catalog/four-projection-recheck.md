...
> **⚠️ Implementation status (2026-06-24): the design-A VERDICT below STANDS (PASSED) and is RATIFIED — but the W2 IMPLEMENTATION that acted on it was merged (#309) then REVERTED (#311) after a prod incident.** The revert was OPERATIONAL — the VO was cut without the PAC-1 data-migration, so pre-W2 `byProfile` loads 500'd — NOT a design flaw. Design A is unchanged and re-applies as-is (with the data-migration this time). See `state.md` · `decisions.md` D-8 · `deferred.md` PAC-18 / PAC-19. _(This file was deleted by the #311 revert and restored at the 2026-06-24 board sync.)_
