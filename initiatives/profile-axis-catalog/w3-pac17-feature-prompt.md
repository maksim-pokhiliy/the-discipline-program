...
**For an EXECUTOR session — W3-core is BUILT + MERGED** (`feat/profile-axis-w3-core` → main); run this AFTER W3-core's owner-gated steps land (the gated api-server test run + the PAC-2 prod-snapshot dry-run + cutover — see `state.md`). Open → `/initiative-resume` (read **decisions D-10** — the ratified decision + the four-projection note; **D-7 §B.3 / §B.6**; **D-1**) → then run `/feature small` with the brief below. This is a fast-follow mini-feature on a DISTINCT surface (the athlete SESSION, not the profile screen) with ZERO shared code with W3-core's athletic card. **The decision-first gate is already DISCHARGED in D-10 — do NOT re-litigate it; build to it.** Owner-smoke after.
...
- **A new `"profile_attribute"` editor kind in `use-session-logging.ts`.** Extend the `ActiveEditor` union (today `{kind:"one_rm"} | {kind:"profile"}`) with `{ rowId; kind: "profile_attribute" }`; add `openProfileAttributeEditor(rowId)` (sibling of `openProfileEditor`) + the `pickGender` callback below; expose both on `SessionEditorControls` + `SessionLogging`.
- **Flip the no-op + add the Popover branch in `schema-row.tsx`.** The `case "profile_attribute"` in `openPrompt` (today `return;`, line ~76) sets the anchor + calls `editor.openProfileAttributeEditor(row.rowId)` — mirror the `case "profile"` path (lines ~71-75). Add a THIRD Popover branch `editorKind === "profile_attribute" && row.load !== null` rendering the gender picker, next to the `one_rm` + `profile` branches (lines ~218-235). Make the steer actionable: the grey `profile_attribute` `Typography` (lines ~163-173) becomes a `LoadPromptButton`-style affordance that opens the picker (today only the non-`profile_attribute` prompt gets a button) — owner-approve the exact affordance on the mock.
- **A gender picker** (a sibling of `inline-profile-picker.tsx`) that filters `binding === "GENDER"` from `row.load.axes`, renders that axis's `values` (`["Male","Female"]`) via the **session** `ProfileOptionButton` (`athlete-session/components/profile-option-button.tsx`), and on pick calls `pickGender`. **Values + axisId come from `row.load`** — exactly as `InlineProfilePicker` reads `load.axes`. **NO `ResolvedLoad` / contract change** (D-10 corrected the older "the arm must carry values+axisId" note — the picker reads the LOAD). Structural sibling on the profile side W3-core just shipped: `athlete-profile/components/profile-pick-group.tsx` (label + values→`ProfileOptionButton`) — mirror its shape, not its module.
- **`pickGender` in `use-session-logging.ts`** — SIMPLER than `pickProfile`: gender is a SINGLE value (no multi-axis staging / `isFullPick` / `stagedProfile` merge). Guard `updateProfile.isPending`; map the picked value → the enum via the registry; `updateProfile.mutate({ gender }, { onSuccess: () => { invalidateView(); setActiveEditor(null); } })`. Reuse the existing mutation — NO new backend.
...
- **The session surface (all under `apps/platform/src/modules/athlete-session/`):** `components/schema-row.tsx` (the `openPrompt` `case "profile_attribute"` no-op + the steer render + the Popover) · `components/inline-profile-picker.tsx` (the plain picker to MIRROR) · `components/profile-option-button.tsx` (the reused primitive — **NB: W3-core added a byte-identical copy at `athlete-profile/components/profile-option-button.tsx`; reuse the SESSION one here**) · `utils/athlete-session-presentation.ts` (the `LoadPrompt` type + the `missing_profile_attribute`→`profile_attribute` construction) · `utils/use-session-logging.ts` (the `ActiveEditor` union + `pickProfile` → add `pickGender`) · `utils/athlete-session.constants.ts` (`SET_SEX_STEER_LABEL`).
