...
| PU-06 | bug        | admin            | Fix A REFUTED (guards only) + honest-UI hide for HEAD_COACH                 | S+S  | done | **MERGED** (PR #338 `11842574`; dev-verified)            |
...
| PU-10 | feature    | marketing        | Socials on /contact + footer (+ restore `sameAs`)                           | S    | W4   | READY (links 27.07: YT/TG/IG/Strava brand)               |
...
| PU-14 | initiative | payments         | Storefront subscription commerce: product→plan, buy→auto-enroll, recurring  | —    | W7   | CHARTERED 2026-09-23 → `initiatives/storefront-billing/` |
...
- **W4 — reach & notify** (`/fix` or `/feature small`): PU-09 + PU-10 (links in hand — YT/TG/IG/Strava brand channels, see `triage.md`) + PU-11; then **PU-17** as a follow-on Claude-Design pass over the full template inventory.
...
**Order (tech-lead-set; sequential for now, parallel when it pays):** Fix-A ✅ → W1 (2 items) → W2 → Wd design round (overlappable) → W3 → W4 → W5 → W6 → W7.
