...
| 0.2 | Legacy prod dump — fast path: Vladyslav's daily Dropbox dump (AS-6); fallback/finish: SSH `pg_dump` of BOTH schemas (prod + `/dev-api`) | owner ops + session | dump restored into the harness DB; row counts sane; freshness + both-schemas coverage verified | pending                                                      |
| 0.3 | Access inventory from Vladyslav: VPS SSH, DNS control confirmation, App Store Connect (the latter = AS-1, not a cutover blocker)        | owner ops           | VPS + DNS access verified hands-on                                                             | 🔵 DNS half done (read-only MCP; snapshot in journal)        |
