...
- Two helpers — `wrapHandler` (public/anonymous) and `wrapAuthHandler` (auth-aware) — are consumed _inside_ the existing factories. Public factory signatures are unchanged. The 113 mutation route call sites compile byte-identical and gain idempotency for free.
- Authenticated routes scope by `user:<userId>`; the single public mutation route (`POST /api/public/contact`) scopes by `ip:<getClientIp(request)>`, mirroring the rate-limit IP scope.
- Body fingerprint is `sha256` over the raw request body bytes (or, for multipart, a stable composition of sorted field names + byte lengths + per-field sha256). Same key with a different fingerprint returns `409 Conflict`.
