...
**Статус:** В работе — 1.1 (ADR) + 1.2.A (BOUNDED-CONTEXTS.md) + 1.2.B (contracts reorg) + 1.2.C (endpoints reorg + authz split + context-util sweep) + 1.2.D (api-server subpath exports) завершены, 1.2.E/F/G и 1.3–1.6 впереди. Новые буллеты, поднятые в 1.2.C: 1.2.E (mappers reorg), 1.2.F (symbol rename), 1.2.G (blog read extraction).
...
| 1.2.F | —           | ⏳ Next | Rename api-server public API symbols to context/role convention (`adminBlogApi → cmsBlogAdminApi`, etc.). 24 symbols, 56 consumer files. Sequenced after 1.2.D.                                     |
