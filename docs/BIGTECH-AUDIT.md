...
**Статус:** В работе — 1.1 (ADR) + 1.2.A (BOUNDED-CONTEXTS.md) + 1.2.B (contracts reorg) + 1.2.C (endpoints reorg + authz split + context-util sweep) + 1.2.D (api-server subpath exports) + 1.2.E (mappers reorg + split) + 1.2.F (api-server symbol rename) + 1.2.G (cms/blog/public.ts extraction) + 1.2.H (marketing client API mirror) + 1.2.I (LMS→Coaching leak fix via plan-roster split) завершены, 1.2.J (IAM→Coaching leak) и 1.3–1.6 впереди. Новые буллеты, поднятые в 1.2.C: 1.2.E (mappers reorg), 1.2.F (symbol rename), 1.2.G (blog read extraction). Новый буллет из 1.2.G: 1.2.H (marketing client API mirror). Буллеты 1.2.I и 1.2.J выделены из существующих research-находок (leaks 98 и 99) для tracking в implementation plan table.
...
| 1.2.I | _pending_   | ✅ Done | Close LMS→Coaching leak: split `planEnrollmentSchema` into pure LMS + enriched `coaching/plan-roster`. `lmsPlanEnrollmentApi` keeps mutations, new `coachingPlanRosterApi` owns reads. Mapper-level leak closes with the same fix.                                   |
| 1.2.J | —           | Pending | Close IAM→Coaching leak: move `adminUserSchema` + `adminUserListItemSchema` out of `iam/user/` (projection-leak importing `athleteProfileSchema`/`coachProfileSchema` from `coaching/`). Mapper-level leak in `iam/user.mapper.ts` closes with the same fix.         |
