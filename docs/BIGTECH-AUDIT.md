...
**Статус:** В работе — 1.1 (ADR) + 1.2.A (BOUNDED-CONTEXTS.md) + 1.2.B (contracts reorg) + 1.2.C (endpoints reorg + authz split + context-util sweep) + 1.2.D (api-server subpath exports) + 1.2.E (mappers reorg + split) + 1.2.F (api-server symbol rename) + 1.2.G (cms/blog/public.ts extraction) + 1.2.H (marketing client API mirror) + 1.2.I (LMS→Coaching leak fix via plan-roster split) + 1.2.J (IAM→Coaching leak fix via admin-user-view split) + 1.2.K (mapper test cleanup) завершены, все 1.2-буллеты закрыты. 1.3 (dependency-cruiser + CI) и 1.4–1.6 впереди.
...
| 1.2.K | _pending_   | ✅ Done | Test cleanup: move `describe("mapToAthleteProfile")` and `describe("mapToCoachProfile")` blocks out of `mappers/iam/user.mapper.test.ts` into per-mapper test files in `mappers/coaching/`. 1.2.E leftover surfaced during 1.2.J test split.                         |
