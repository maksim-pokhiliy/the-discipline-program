...
| 1.4.B | —           | Deferred to §2 | **Deferred — closes together with §2 Money value object.** `centsToAmount` / `amountToCents` / `CENTS_PER_UNIT` are helpers of a missing `Money` value object, not standalone primitives. Moving them into `@repo/contracts/common/money` as a standalone step would place behavior inside the API-boundary layer (ADR 0005: contracts = schemas + types, not behavior) and then move them again when §2 lands. Double work, no M7 review credit. Do not execute as a standalone bullet — §2 Money VO bullet owns this scope. |
...
- [ ] **Value Objects.** `Money` не должен быть `number`. `centsToAmount` есть, но это helper, а не тип. `Money = { amount: number; currency: Currency }` — явный тип, который нельзя сложить с обычным числом. Currency enum уже есть (`USD / EUR / UAH` — multi-currency planned → удваивает сложность). То же для `Email`, `Cuid`, `Slug`, `DurationSeconds`. **Owns §1.4.B scope:** the Money VO bullet also moves `centsToAmount` / `amountToCents` / `CENTS_PER_UNIT` out of `@repo/shared/helpers/money.ts` to live next to the new value object as constructors/converters. 1.4.B was deferred to §2 after a standalone-move attempt was reverted — see the §1.4 bullet and the 1.4.B implementation plan row.
