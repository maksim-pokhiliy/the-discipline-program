...
**Статус:** В работе — 1.1 (ADR) + 1.2.A (BOUNDED-CONTEXTS.md) + 1.2.B (contracts reorg) + 1.2.C (endpoints reorg + authz split + context-util sweep) + 1.2.D (api-server subpath exports) + 1.2.E (mappers reorg + split) + 1.2.F (api-server symbol rename) + 1.2.G (cms/blog/public.ts extraction) завершены, 1.2.H и 1.3–1.6 впереди. Новые буллеты, поднятые в 1.2.C: 1.2.E (mappers reorg), 1.2.F (symbol rename), 1.2.G (blog read extraction). Новый буллет из 1.2.G: 1.2.H (marketing client API mirror).
...
| 1.2.G | _pending_   | ✅ Done | Extract blog reads from `cms/pages/public.ts` into new `cms/blog/public.ts` exposing `cmsBlogPublicApi` (`listPublished()` + `getArticle(slug)`). Pages endpoint delegates, marketing article route switches to `cmsBlogPublicApi`. New finding from 1.2.C research. |
| 1.2.H | —           | Pending | Mirror 1.2.G on marketing client: `apps/marketing/src/lib/api/endpoints/blog.ts` with `getArticle()`, remove `getBlogArticle` from `pages.ts`, update consumers (`use-blog.ts`, `app/blog/[slug]/page.tsx`). Surfaced during 1.2.G.                                  |
