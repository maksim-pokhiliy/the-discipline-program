...
| 1.2.H | _pending_   | ✅ Done | Mirror 1.2.G on marketing client: `apps/marketing/src/lib/api/endpoints/blog.ts` with `getArticle()`, remove `getBlogArticle` from `pages.ts`, update consumers (`use-blog.ts`, `app/blog/[slug]/page.tsx`). Surfaced during 1.2.G.                                  |
