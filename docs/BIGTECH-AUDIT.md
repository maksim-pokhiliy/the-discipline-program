...
**Статус:** В работе — 1.1 (ADR) + 1.2.A–K (все) + 1.3.A (dependency-cruiser boundary rules) завершены. 1.3.B (GitHub Actions CI) + 1.3.C (dep-graph artifact) + 1.4–1.6 впереди. Важно: первый прогон `pnpm dep:check` после 1.3.A дал 0 violations — вся 1.2-работа holds на file-import уровне, section 1.3 работает как intended financial verification layer для structural work of section 1.2.
...
| 1.3.A | _pending_   | ✅ Done | Add `dependency-cruiser` with 17 boundary rules encoding BOUNDED-CONTEXTS.md §8 (contracts leafs, IAM/LMS/CMS direction rules, Prisma isolation, app-scope rules, admin→coaching file-precise carve-out). Wired into lefthook pre-push. First run green: 0 violations on 785 modules / 1409 dependencies. |
