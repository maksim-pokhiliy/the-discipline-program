...
**Статус:** В работе — 1.1 (ADR) + 1.2.A–K (все) + 1.3.A (dependency-cruiser boundary rules) + 1.3.B (GitHub Actions CI) завершены. 1.3.C (dep-graph artifact) + 1.4–1.6 впереди. Важно: после 1.3.B у репо появился первый полноценный CI с 5 параллельными гейтами; первая волна section 1.3 (1.3.A + 1.3.B) закрыла gap между "работа сделана" и "работа защищена".
...
| 1.3.B | _pending_   | ✅ Done | Add `.github/workflows/ci.yml` with 5 parallel lanes (check-types, lint, dep-check, test, build). Test lane uses postgres:16-alpine service container + `prisma db push` for fresh schema. Build lane uses dummy env + `SKIP_ENV_VALIDATION=1`. Concurrency cancels in-flight runs for the same ref.      |
