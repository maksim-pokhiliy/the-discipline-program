...
**Статус:** В работе — 1.1 (ADR) + 1.2.A (BOUNDED-CONTEXTS.md) + 1.2.B (contracts reorg) + 1.2.C (endpoints reorg + authz split + context-util sweep) + 1.2.D (api-server subpath exports) + 1.2.F (api-server symbol rename) завершены, 1.2.E/G и 1.3–1.6 впереди. Новые буллеты, поднятые в 1.2.C: 1.2.E (mappers reorg), 1.2.F (symbol rename), 1.2.G (blog read extraction).
...
| 1.2.E | —           | ⏳ Next | Reorganize `packages/api-server/src/mappers/` by bounded context (`mappers/{cms,lms,coaching,iam}/`). New finding from 1.2.C research.                                                              |
